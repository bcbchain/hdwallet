...
​	首先修改Xwallet配置文件.config/hdWallet.yaml，如果在测试链上使用只需要修改chainID为测试链chainID，nodeAddrSlice改为测试链的接入的URL就可以了。（URL不是域名的需要改为http://ip:46657, rpc服务如果使用的是https需要将useHttps改为true）
...
- **command**
  ...
  ```
  hdWallet transfer --path "m/44'/60'/0'/0/1" --password BYd.. --smcAddress bcbLVgb... 
